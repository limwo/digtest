...
for sentence in sentences:
    ...
    # 计算目标句子与当前句子的相似度得分
    similarity_score = euclidean_distances(model(target_encoding['input_ids'])[1].detach().numpy(),
                                          model(sentence_encoding['input_ids'])[1].detach().numpy()).mean()
    similarity_scores.append(similarity_score)

# 4. 输出相似度前三的句子及其相似度得分
sorted_indices = sorted(range(len(similarity_scores)), key=lambda i: similarity_scores[i], reverse=False)[:3]
